...
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QFileDialog, QGraphicsScene,QCheckBox,
...
class GerberViewer(QMainWindow):
    ...
    displayToggled = pyqtSignal(str, bool)
    ...
    def __init__(self):
        ...
        # Traces checkbox
        self.traces_check = QCheckBox("Show Traces")
        self.traces_check.setChecked(True)
        self.traces_check.toggled.connect(self._on_traces_toggled)
        display_layout.addWidget(self.traces_check)

        # Flashes checkbox
        self.flashes_check = QCheckBox("Show Flashes")
        self.flashes_check.setChecked(True)
        self.flashes_check.toggled.connect(self._on_flashes_toggled)
        display_layout.addWidget(self.flashes_check)

        # Polygons checkbox
        self.polygons_check = QCheckBox("Show Polygons")
        self.polygons_check.setChecked(True)
        self.polygons_check.toggled.connect(self._on_polygons_toggled)
        display_layout.addWidget(self.polygons_check)

        # Apertures checkbox
        self.apertures_check = QCheckBox("Show Aperture Outlines")
        self.apertures_check.setChecked(True)
        self.apertures_check.toggled.connect(
            lambda checked: self.displayToggled.emit('apertures', checked)
        )
        display_layout.addWidget(self.apertures_check)

        # Theme checkbox
        self.theme_check = QCheckBox("Dark Theme")
        self.theme_check.toggled.connect(self.themeToggled.emit)
        display_layout.addWidget(self.theme_check)
        ...
        dock_layout.addWidget(display_group)        
        ...
        self.displayToggled.connect(self.on_display_toggled)
        self.themeToggled.connect(self.on_theme_toggled)
        

    def _on_polygons_toggled(self, checked):
        """
        Show or hide all polygon items (typically board outlines or filled areas).
        """
        print(f"[Signal] Show Polygons toggled: {checked}")
        self.displayToggled.emit('polygons', checked)
        for name, layer in self.layers.items():
            if layer.get("is_outline"):  # Assuming polygons are marked with is_outline=True
                if "group" in layer:
                    layer["group"].setVisible(checked)
                
    def _on_traces_toggled(self, checked):
        """
        Show or hide all layers that contain 'trace' or typical copper layers (.gtl, .gbl).
        """
        print(f"[Signal] Show Traces toggled: {checked}")
        self.displayToggled.emit('traces', checked)

        for name, layer in self.layers.items():
            lname = name.lower()
            # Identify likely "trace" layers by filename or content
            if any(ext in lname for ext in [".gtl", ".gbl", "copper", "trace"]):
                if "group" in layer:
                    layer["group"].setVisible(checked)

    def _on_flashes_toggled(self, checked):
        """
        Show or hide all flashed items (pads, vias, etc.).
        For simplicity, this will hide ellipse and rect items from scene.
        """
        print(f"[Signal] Show Flashes toggled: {checked}")
        self.displayToggled.emit('flashes', checked)
    ...
    def on_display_toggled(self, name, state):
        print(f"Display option changed: {name} = {state}")
        if name == 'polygons':
            for lname, layer in self.layers.items():
                if layer.get("is_outline") and "group" in layer:
                    layer["group"].setVisible(state)


    def on_theme_toggled(self, checked):
        print(f"Theme toggled: {'Dark' if checked else 'Light'}")
        if checked:
            self.scene.setBackgroundBrush(QColor(0, 0, 0))
        else:
    ...
    def new_project(self):
        ...
        self.view.setSceneRect(-1000, -1000, 2000, 2000)  # or appropriate default
   ...
   # -----------------------------
        ...
        for x, y, dia in drills:
            ...
            try:
                cylinder = trimesh.creation.cylinder(radius=dia / 2.0,
                                                     ...
                                                     sections=32)
                cylinder.apply_translation([x, y, 1.1*HOLE_HEIGHT / 2.0])
    ...
    def open_files(self):
        ...
        for path in paths:
            ...
            lname = name.lower()
            if lname.endswith(".gtl") or "top" in lname:
                color = QColor(255, 0, 0)  # red top copper
            elif lname.endswith(".gbl") or "bottom" in lname:
    ...
    def parse_gerber(self, filepath, scene_factor=1.0):
        """
        Parse a Gerber RS-274X file into QPainterPath objects or QGraphicsItems.

        Handles:
          - %MO (unit)
          - %FS (format specification)
          - %ADD (aperture definition)
          - %AM (aperture macros, simplified)
          - %LP (polarity)
          - %SR (step and repeat)
          - G01/G02/G03 (draw modes)
          - G36/G37 (region polygons)
          - D01/D02/D03 (draw/move/flash)
          - Arcs (CW/CCW with I/J)
        """
        ...
        step_repeat = (1, 1, 0.0, 0.0)
        format_decimals = (2, 4)  # default X2.4

        # --- Helper functions ---
        def parse_number(num_str):
            """Convert coordinate string according to FS format"""
            try:
                sign = -1 if num_str.startswith('-') else 1
                num_str = num_str.lstrip('+-')
                int_part = num_str[:-format_decimals[1]] if len(num_str) > format_decimals[1] else '0'
                dec_part = num_str[-format_decimals[1]:] if len(num_str) > 0 else '0'
                return sign * (float(int_part) + float(dec_part) / (10 ** len(dec_part)))
            except Exception:
                return 0.0

        def add_item(it):
            """Apply polarity and step-repeat"""
            nx, ny, dx, dy = step_repeat
            for ix in range(nx):
                for iy in range(ny):
                    copy = it
                    if isinstance(it, QPainterPath):
                        path = QPainterPath(it)
                        path.translate(ix * dx, iy * dy)
                        if not polarity_positive:
                            # Negative polarity could be handled as clearing regions
                            pass
                        items.append(path)
                    elif hasattr(it, "setPos"):
                        dup = it
                        dup.setPos(it.pos().x() + ix * dx, it.pos().y() + iy * dy)
                        items.append(dup)
                    else:
                        items.append(it)
        ...
        with open(filepath, 'r') as f:
            for raw in f:
                ...
                if line.startswith('%FS'):
                    ...
                    if m:
                        format_decimals = (int(m.group(1)), int(m.group(2)))
                    continue
                ...
                if line.startswith('%MO'):
                    if "IN" in line.upper():
                        unitfactor = 25.4
                    elif "MM" in line.upper():
                        unitfactor = 1.0
                    continue

                # --- Polarity ---
                if line.startswith('%LP'):
                    polarity_positive = 'D' in line.upper()
                    continue
                ...
                if line.startswith('%SR'):
                    ...
                    if m:
                        step_repeat = (int(m.group(1)), int(m.group(2)),
                                       float(m.group(3)) * unitfactor,
                                       float(m.group(4)) * unitfactor)
                    continue

                # --- Aperture Macros (simplified) ---
                if line.startswith('%AM'):
                    macro_name = line[3:].split('*')[0].strip()
                    macro_body = []
                    while not line.endswith('*%'):
                        line = next(f).strip()
                        macro_body.append(line)
                    self.aperture_macros[macro_name] = ('MACRO', macro_body)
                    continue
                ...
                if line.startswith('G02'):
                    interpolation_mode = 'cw_arc'
                    continue
                if line.startswith('G03'):
                    interpolation_mode = 'ccw_arc'
                    continue
                if line.startswith('G74') or line.startswith('G75'):
                    continue  # ignore single/multi quadrant arcs
                ...
                if mdc:
                    ...
                    if code in self.aperture_macros:
                ...
                for axis, num_s in coords:
                    vals[axis] = parse_number(num_s) * unitfactor
                    ...
                    if polygon_mode and polygon_path:
                        ...
                        if interpolation_mode in ('cw_arc', 'ccw_arc'):
                            ...
                            if path:
                                ...
                                add_item(stroker.createStroke(path))
                        else:
                            path = QPainterPath()
        ...
        # --- Flip Y (Gerber coordinate system) ---
